...
<div class="crm-table-wrap" style="overflow:visible;">
    ...
    <div class="crm-table-toolbar">
        ...
        <div style="margin-left:auto;">@include('crm.partials.per-page-selector', ['paginator' => $invoices])</div>
    </div>
            ...
            <tr onclick="window.location='{{ route('crm.invoices.show', $inv) }}'" style="cursor:pointer;">
                ...
                <td onclick="event.stopPropagation()" style="white-space:nowrap;">
                    <div x-data="{ open: false, sendOpen: false }" style="position:relative;display:inline-block;">
                        ...
                        @if($inv->status === 'Draft' && $inv->client?->email)
                        <div x-show="sendOpen" class="crm-modal-overlay" @click.self="sendOpen=false">
                            <div class="crm-modal" @click.stop style="max-width:420px;">
                                ...
                                <div class="crm-modal-body">
                                    <p style="font-size:0.9375rem;color:var(--color-ink-2);">Send <strong>{{ $inv->invoice_id }}</strong> to <strong>{{ $inv->client->email }}</strong>?</p>
                                </div>
                                <div class="crm-modal-footer">
                                    <button type="button" @click="sendOpen=false" class="crm-btn crm-btn-secondary">Cancel</button>
                                    <form method="POST" action="{{ route('crm.invoices.send', $inv) }}" style="display:inline;">
                                        @csrf
